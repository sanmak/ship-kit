...
### readme-footer (feature) — 2026-03-15
2 tasks completed. Added `add_readme_footer()` function to `install.sh` and Step 0 pre-check to `commands/ship.md`. Covers all install methods: script installs get footer during install, plugin/clone installs get footer on first `/ship` run. Deviated from original design by adding ship.md coverage per user request.
